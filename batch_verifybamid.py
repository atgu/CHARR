    ...
    with open(f"{local}/contam_temp.selfSM", "rt") as selfSM:
        ...
        for row in reader:
            if float(row["FREELK0"]) == 0 and float(row["FREELK1"]) == 0:
                ...
                sys.exit(1)
            print(
                f'{output_prefix} contamination rate: {float(row["FREEMIX"]) /contamination_underestimation_factor}'
            )
            ...
            if i != 1:
                sys.stderr.write(
                    "Found %d rows in .selfSM file. Was expecting exactly 1. This is an error"
                    % (i)
                )
                sys.exit(2)
    return c
...
def run_verifybamid(
    ...
    if hl.hadoop_exists(input_cram_path):
        ...
        bam_size = bucket.get_blob(
            input_cram_path.replace(f"gs://{bucket_name}/", "")
        ).size
    else:
        raise ValueError(f"Input cram file {input_cram_path} does not exist!")

    if hl.hadoop_exists(ref_fasta_path):
        client: storage.client.Client = storage.Client("bigquery-public-data")
        bucket: storage.bucket.Bucket = client.get_bucket(
            "gcp-public-data--broad-references"
        )
        ref_size = bucket.get_blob("hg38/v0/Homo_sapiens_assembly38.fasta").size
    else:
        raise ValueError(f"Input reference file {ref_fasta_path} does not exist!")

    ncpu = 8  # ~ 8G/core ~ 64G
    disk_size = ceil((bam_size + ref_size) / (1024**3)) + 30

    sample_id = "_".join(output_prefix.split("_")[:-1])
    chromosome = output_prefix.split("_")[-1]
    if len(output_prefix.split("_")) == 4:
        s1 = output_prefix.split("_")[0]
        s2 = output_prefix.split("_")[1]
        contam_rate = output_prefix.split("_")[2]
        chromosome = output_prefix.split("_")[3]
        attributes = {
            "s1": s1,
            "s2": s2,
    ...
    logger.info(f"Requesting storage: {disk_size} GiB ... ")
...
def main(args):
    ...
    try:
        ...
        if args.test:
            ...
            ht = hl.import_table(
                "gs://gnomad-wenhan/verifybamid/gnomad_v3_hgdp_low_contam.txt"
            ).key_by("s")
            cram_data = ht.aggregate(
                hl.struct(
                    cram_path=hl.agg.collect(ht.final_cram_path),
                    crai_path=hl.agg.collect(ht.final_crai_path),
                    sample_ids=hl.agg.collect(ht.s),
                )
            )
            if args.run_verifybamid:
                ...
                for cram_path, crai_path in zip(
                    cram_data.cram_path, cram_data.crai_path
                ):
                    run_verifybamid(
                        b=b,
                ...
                contam_paths = [
                    tmp_bucket + x.split("/")[-1][:-5] + ".selfSM"
                    for x in cram_data.cram_path
                ]
                contam_est = []
                for name in contam_names:
                    contam_est.append(
                        check_contam(
                            args.contamination_underestimation_factor, tmp_bucket, name
                        )
                    )
                cram_data = cram_data.annotate(
                    recomputed_contam_rate=contam_est, contam_path=contam_paths
                )
                cram_data.write(
                    f"{tmp_bucket}/{args.output_contam_table_name}.ht", overwrite=True
                )
